...
(function ($) {
    $.fn.slideShow = function (options) {
        ...
        this.defaults = {start:0, interval:3, repeat:true, transition:{mode:'fade', speed:1000}, slideSize:'auto', hoverNavigation:false, slideClick:false, gotoSlide:false, mousePause:false};
        ...
        this.init = function () {
            if (this.find('.slides').length) {
                if (this.options.slideSize == 'auto') {
                    this.options.slideSize = {width:this.find('.slide:first img').width(), height:this.find('.slide:first img').height()};
                }
                if (this.options.slideSize != 'none' && this.options.slideSize != false) {
                    this.find('.slides').css({height:this.options.slideSize.height + 'px', width:this.options.slideSize.width + 'px', overflow:'hidden'});
                }
            }
            ...
            this.find('.slide').mouseenter(function () {
                ...
                if (!slideShow.options.mousePause) {
                    slideShow.stopAuto();
                }
            });
        ...
        this.gotoSlide = function (index, noanimation) {
            ...
            this.find('.slides').animate({height:newSlide.height()});
